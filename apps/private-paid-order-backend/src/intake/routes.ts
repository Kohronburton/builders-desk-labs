...
import type { IntakeRepository, WebhookAttemptFinish } from "./repository.js";
import { verifyWebhook, type KeyResolver, type NonceStore, type WebhookHeaders } from "../security/webhook.js";

export interface PaidOrderRouteOptions {
...
export async function registerPaidOrderRoutes(app: FastifyInstance, options: PaidOrderRouteOptions): Promise<void> {
  // This parser exists only inside the encapsulated webhook plugin. Other API
  // routes retain Fastify's normal JSON-object parser.
  ...
  app.post("/api/v1/webhooks/wordpress/paid-orders", async (request, reply) => {
    ...
    try {
      ...
      options.logger.log("error", "webhook attempt could not be recorded", { requestId, error: error instanceof Error ? error.message : "unknown" });
    ...
    const finish = async (fields: Omit<WebhookAttemptFinish, "requestId" | "processingDurationMs">) => {
      try {
        ...
        options.logger.log("error", "webhook attempt finalization failed", { requestId, error: error instanceof Error ? error.message : "unknown" });
      }
    };
    ...
    try {
      ...
      const errors = [{ path: "$", code: "INVALID_JSON", message: "Request body must be valid JSON." }];
      await finish({
        signatureStatus: "verified", timestampStatus: "valid", replayStatus: "clear", validationStatus: "rejected",
        finalStatus: "rejected", httpResponseCode: 422, failureCode: "INVALID_JSON", failureDetails: errors
      });
    ...
    const parityErrors: Array<{ path: string; code: string; message: string }> = [];
    ...
    try {
      ...
      options.logger.log("error", "paid order acceptance failed", { requestId, externalOrderId: order.order.externalOrderId, error: error instanceof Error ? error.message : "unknown" });
