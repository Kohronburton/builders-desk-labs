...
setupForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    renderStatus(data);
    document.querySelector('#providerMode').value = 'auto';
    setupStatus.insertAdjacentHTML('beforeend', '<em>Saved in server memory. Run the next request to verify the live model.</em>');
  } catch (error) {
...
clearSetup?.addEventListener('click', async () => {
  const setupToken = setupForm.setupToken.value;
  const response = await fetch('/api/provider-config', {
    method: 'DELETE',
    headers: { 'content-type': 'application/json' },
